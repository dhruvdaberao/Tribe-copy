...
const ChatPage: React.FC<ChatPageProps> = ({ currentUser, allUsers, emberUser, initialTargetUser, onViewProfile }) => {
  ...
  useEffect(() => {
    ...
    const handleUserTyping = ({ user }: { user: User }) => {
        setTypingUsers(prev => [...prev.filter(u => u.id !== user.id), user]);
    };

    const handleUserStoppedTyping = ({ user }: { user: User }) => {
  ...
  const handleBackToList = () => {
    const otherUserId = activeConversation?.participants.find(p => p.id !== currentUser.id)?.id;
    if (otherUserId) {
        socket?.emit('leave-room', otherUserId);
    }
    setActiveConversation(null);
  };

  const handleSendMessage = async (text: string) => {
    if (!activeConversation || isSending || !socket) return;
    ...
    const tempMessage: Message = { id: `temp-${Date.now()}`, senderId: currentUser.id, text, timestamp: new Date().toISOString() };
    setMessages(prev => [...prev, tempMessage]);
    socket.emit('stop-typing', { roomId: currentUser.id, user: currentUser });
    ...
    if (otherUserId === emberUser.id) {
        try {
            ...
            console.error("Ember AI Error:", error);
            const errorMessage: Message = { id: `ember-err-${Date.now()}`, senderId: emberUser.id, text: "Sorry, I'm having a little trouble thinking right now. Please try again in a moment.", timestamp: new Date().toISOString() };
    ...
    try {
        const { data: sentMessage } = await api.sendMessage(otherUserId, { message: text });
